...
const TEMPO_ESPERA = 4 * 60 * 60 * 1000; // 4 horas em ms

// Carrega e limpa vilas antigas
function carregarVilasValidas() {
    const agora = Date.now();
    const dados = JSON.parse(localStorage.getItem(STORAGE_KEY) || '{}');

    // Remove entradas com mais de 4 horas
    ...
    // Salva limpo novamente
    localStorage.setItem(STORAGE_KEY, JSON.stringify(dados));
    return dados;
}

// Marca vila como usada
...
const vilasParaUparFiltrado = vilasParaUpar.filter(([coord]) => !vilasUpando.includes(coord));

const rotas = distribuirRecursos(vilasParaUparFiltrado, vilasFinalizadas, vilasNaoFinalizadas);
const rotasComId = substituirCoordenadaPorId(rotas,VillagesIDs);
/*console.log(vilasUpando);
console.log(vilasUsadas);
